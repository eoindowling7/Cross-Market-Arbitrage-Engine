"""Paper engine v3: separates immediate and passive opportunities.
...
def _fixed_taker_price(candidate, poly_market, series_info, max_skew_seconds):
    ...
    kbook = get_market_orderbook(candidate["ticker"], depth=100)
    kalshi_received = time.monotonic()
    poly_yes = _poly_ask_levels(tokens[0])
    poly_no = _poly_ask_levels(tokens[1])
    poly_received = time.monotonic()
    fetch_skew = poly_received - kalshi_received
...
def run_v3(engine_config, confirmation_config):
    ...
    for idx, candidate in enumerate(selected, 1):
        ...
        confirmations.append(
            confirm_candidate(candidate, pm, series_cache[series], engine_config, confirmation_config)
        )
