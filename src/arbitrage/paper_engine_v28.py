"""V28 execution-recovery semantic-hybrid paper engine (paper only).
...
from src.arbitrage.execution_recovery_v28 import classify_exception, late_hydrate_candidate
from src.arbitrage.fast_recycle_v14 import FastRecycleConfig, scan_polymarket_complete_sets
from src.arbitrage.multi_venue_discovery import audit_extra_venues
from src.arbitrage.multi_outcome_basket_v14 import MultiOutcomeConfig, scan_polymarket_multi_outcome
from src.arbitrage.limitless_polymarket_v14 import LimitlessPolyConfig, scan_limitless_polymarket
...
# V29 paper-research switches. These are intentionally environment-gated so
# V28.4 behavior remains unchanged unless the V29 wrapper enables them.
V29_RELAX_UNRESOLVED = os.getenv("V29_RELAX_UNRESOLVED", "0") == "1"
V29_SKIP_LATE_RULE_HYDRATION = os.getenv("V29_SKIP_LATE_RULE_HYDRATION", "0") == "1"
V29_ALLOW_ONE_SIDED_STRUCTURAL = os.getenv("V29_ALLOW_ONE_SIDED_STRUCTURAL", "0") == "1"




def _refresh_poly_map_v281(previous=None):
    """Refresh Polymarket metadata without allowing a catastrophic downgrade.

    The API client already preserves its last-known-good disk cache.  This
    second in-memory guard protects a live run even if an upstream caller
    returns an unexpectedly small universe for any reason.
    """
    markets = get_active_markets(limit=None)
    fresh = {str(m.get("question")): m for m in markets if m.get("question")}
    previous = previous or {}
    if previous and len(previous) >= 10000:
        minimum = max(10000, int(len(previous) * 0.50))
        if len(fresh) < minimum:
            print(
                "V28.1 Polymarket in-memory downgrade guard | "
                f"new={len(fresh)} old={len(previous)} minimum={minimum} | keeping old map"
            )
            return previous
    return fresh


def _candidate_poly_snapshot(candidate):
    snap = candidate.get("_poly_market_snapshot") if isinstance(candidate, dict) else None
    return snap if isinstance(snap, dict) and snap.get("question") else None
...
class V8Config(V7Config):
    ...
    strict_yield_sleeve_fraction: float = 0.40
    strict_yield_min_apr: float = 0.040
    ...
    max_event_fraction: float = 0.30
    reserve_cash_fraction: float = 0.20
    ...
    opportunity_cost_apr: float = 0.060
    ...
    benchmark_target_apr: float = 0.060
    ...
    fast_recycle_market_limit: int = 6000
    ...
    multi_outcome_max_days: float = 60.0
    multi_outcome_max_events: int = 250
    multi_outcome_event_fraction: float = 0.10
    multi_outcome_min_roc: float = 0.0010
    ...
    limitless_cross_max_days: float = 21.0
    ...
    low_basis_total_fraction: float = 0.03
    low_basis_market_fraction: float = 0.010
    low_basis_event_fraction: float = 0.020
    low_basis_max_reserve_per_contract: float = 0.06
    low_basis_min_adjusted_npc: float = 0.005
    low_basis_min_adjusted_apr: float = 0.060
    ...
    maker_probe_min_expected_apr: float = 0.060
...
# Public final-candidate name; V8Config is retained internally for compatibility.
V14Config = V8Config
...
class RunLoggerV8(RunLoggerV7):
    def __init__(self, run_id: str):
        ...
        self.paths.update({
            ...
            "near_miss": self.root / "near_miss_ledger.csv",
            # V28.3: late execution-recovery diagnostics. V28.2 wrote
            # log.append("recovery", ...) but never registered the path,
            # causing a KeyError on the first recovery event.
            "recovery": self.root / "candidate_recovery.csv",
        })
...
def _optimized_fresh_signal(candidate: dict, pm: dict, series_info: dict, *, max_qty: int, capital_limit: float, cfg: V8Config):
    ...
    # V28: obtain both venue snapshots concurrently.  We preserve the same
    # quote-skew ceiling; concurrency reduces artificial skew from sequential
    # network waits instead of relaxing execution realism.
    def _fetch_k():
        levels = _kalshi_taker_asks(candidate["ticker"], kalshi_side)
        return levels, time.monotonic()
    def _fetch_p():
        levels = _poly_asks(poly_token)
        return levels, time.monotonic()
    with ThreadPoolExecutor(max_workers=2) as pool:
        kf = pool.submit(_fetch_k)
        pf = pool.submit(_fetch_p)
        kasks, kt = kf.result()
        pasks, pt = pf.result()
    skew = abs(pt - kt)
    if skew > cfg.max_quote_skew_seconds:
        return None, f"quote skew {skew:.3f}s above {cfg.max_quote_skew_seconds:.3f}s"
    if not kasks and not pasks:
        return None, "temporary book unavailable: both executable sides empty"
    if not kasks:
        return None, "temporary book unavailable: Kalshi executable side empty"
    if not pasks:
        return None, "temporary book unavailable: Polymarket executable side empty"
...
def _safe_optimized_signal(candidate, pm, series_info, *, max_qty, capital_limit, cfg: V8Config):
    ...
    for attempt in range(cfg.api_retries + 1):
        try:
            signal, reason = _optimized_fresh_signal(
                candidate, pm, series_info, max_qty=max_qty, capital_limit=capital_limit, cfg=cfg
            )
            # V29: an empty executable side is a normal transient market state,
            # not a terminal failure for the cycle. Give it the same bounded
            # retry/backoff treatment as transient API exceptions.
            if signal is None and str(reason).startswith("temporary book unavailable"):
                last_reason = reason
            ...
            if transient and attempt < cfg.api_retries:
                time.sleep(cfg.api_retry_backoff_seconds * (2 ** attempt))
                continue
            prefix = "temporary API/book failure" if transient else "permanent API/book failure"
            return None, f"{prefix}: {reason}"
    if last_reason is not None:
        return None, last_reason
    reason, transient = classify_exception(last)
    prefix = "temporary API/book failure" if transient else "permanent API/book failure"
    return None, f"{prefix}: {reason}"
...
def _equivalence_edge_gate(candidate: dict, signal: dict, cfg: V8Config) -> tuple[bool, str]:
    ...
    if ko or po:
        if not ko or not po:
            if not V29_ALLOW_ONE_SIDED_STRUCTURAL:
...
def _merge_confirmed_watch(watch: list, engine_config: V2Config, confirmation_config: ConfirmationConfig,
    ...
    previous_map = {str(item[1].get("question")): item[1] for item in watch if isinstance(item[1], dict) and item[1].get("question")}
    by_question = _refresh_poly_map_v281(previous_map)
        ...
        if key in existing:
            ...
            for item in watch:
                if (item[0].get("ticker"), item[0].get("strategy")) == key:
                    pm = by_question.get(c.get("poly_question")) or _candidate_poly_snapshot(c)
                    if pm is not None:
                        item[0] = c
                        item[1] = pm
                    break
...
def run_v8(engine_config: V2Config, confirmation_config: ConfirmationConfig, cfg: V8Config):
    ...
    print(f"V28.2 PERSISTENT WATCH run id: {run_id}")
    print("Building V14 final-candidate expanded-market universe...")
    ...
    # V28.1: build a best-effort global map, but confirmed candidates also
    # carry the exact Polymarket snapshot used during confirmation.
    by_question = _refresh_poly_map_v281()
    ...
    for c in confirmed:
        ...
        if series not in series_cache:
            series_cache[series] = get_series_info(series)
        watch.append([c, pm, series_cache[series]])
    if snapshot_fallbacks:
        print(f"V28.1 watch recovery | preserved_from_confirmation_snapshot={snapshot_fallbacks}")

    rng = random.Random(cfg.random_seed)
    previous_executed_state = {}
    results = []
    rejection_counts = Counter()
    # V28 late-hydration cache. SAFE and explicit contradictions are stable;
    # unresolved entries are retried after a short TTL by the helper.
    execution_recovery_cache: dict = {}
    ...
    persistent_watch_size = len(watch)
    print(
        f"Watching {persistent_watch_size} candidates for {cfg.run_minutes:g} minutes | "
        f"bankroll=${engine_config.bankroll:.2f} | velocity allocator ON"
    )
    print(
        f"V28.2 persistent watch locked | admitted={persistent_watch_size} | "
        "temporary failures will NOT remove candidates between cycles"
    )
    ...
    while time.monotonic() < deadline:
        ...
        if (time.monotonic() - last_metadata_refresh) >= cfg.metadata_refresh_minutes * 60:
            try:
                refreshed_map = _refresh_poly_map_v281(by_question)
                by_question = refreshed_map
                for item in watch:
                    refreshed = by_question.get(item[0].get("poly_question"))
                    if refreshed is not None:
                        item[1] = refreshed
                    elif item[1] is None:
                        item[1] = _candidate_poly_snapshot(item[0])
                last_metadata_refresh = time.monotonic()
        ...
        if cfg.universe_refresh_minutes > 0 and (time.monotonic() - last_universe_refresh) >= cfg.universe_refresh_minutes * 60:
            # V28.2: the confirmation pass is the FINAL admission check for this run.
            # Never rebuild, shrink, replace, or add to the watch membership after
            # ``Confirmed watch candidates`` is printed. We may refresh metadata for
            # those exact candidates, but temporary discovery/API failures cannot
            # remove them from subsequent cycles.
            try:
                refreshed_map = _refresh_poly_map_v281(by_question)
                by_question = refreshed_map
                refreshed_count = 0
                ...
                for item in watch:
                    ...
                    if refreshed is not None:
                        ...
                        # Keep the last-known-good metadata already stored on the
                        # persistent watch item; fall back to its confirmation snapshot
                        # only if somehow missing.
                        if item[1] is None:
                            item[1] = _candidate_poly_snapshot(item[0])
                        snapshot_kept += 1
                last_universe_refresh = time.monotonic()
                ...
                print(
                    f"V28.2 persistent watch refresh | membership={len(watch)} unchanged | "
                    f"metadata_refreshed={refreshed_count} | last_good_kept={snapshot_kept}"
                )
            except Exception as exc:
                # Most importantly: leave ``watch`` completely untouched.
                last_universe_refresh = time.monotonic()
                print(
                    f"V28.2 persistent watch refresh warning: {type(exc).__name__}: {exc} | "
                    f"membership={len(watch)} preserved"
                )
        ...
        if cfg.fast_recycle_enabled and (time.monotonic() - last_fast_recycle_scan) >= cfg.fast_recycle_scan_seconds:
            ...
            try:
                ...
                for rank, miss in enumerate(fstats.get("near_misses") or [], start=1):
                    log.append("near_miss", {"timestamp": time.time(), "lane": "complete_set_merge",
                        "rank": rank, **miss})
                for fr in fast:
        ...
        if cfg.multi_outcome_enabled and (time.monotonic() - last_multi_outcome_scan) >= cfg.multi_outcome_scan_seconds:
            ...
            try:
                ...
                for rank, miss in enumerate(bstats.get("near_misses") or [], start=1):
                    log.append("near_miss", {"timestamp": time.time(), "lane": "multi_outcome_basket",
                        "rank": rank, **miss})
        ...
        if cfg.limitless_cross_enabled and (time.monotonic() - last_limitless_cross_scan) >= cfg.limitless_cross_scan_seconds:
            ...
            try:
                ...
                for rank, miss in enumerate(lstats.get("near_misses") or [], start=1):
                    log.append("near_miss", {"timestamp": time.time(), "lane": "limitless_polymarket",
                        "rank": rank, **miss})
                for op in lops:
        ...
        proposals = []
        # V28.2: every admitted candidate is revisited on every cycle. A temporary
        # book/API/rule/edge failure affects only this cycle's trade eligibility.
        # It never mutates watch membership.
        for candidate, pm, series_info in watch:
            cycle_watch_checked += 1
            ...
            if signal is None:
                rejection_counts[reason] += 1
                log.append("signals", _signal_row_v8(candidate, None, False, reason, settlement_days=rough_days, duration_bucket=bucket))
                if reason.startswith("temporary API/book failure"):
            ...
            if signal["net_profit"] < cfg.min_trade_net_dollars:
                ...
                log.append("signals", _signal_row_v8(candidate, signal, False, reason, settlement_days=rough_days, duration_bucket=bucket))
                continue
            # V28: only now, after a live positive signal exists, spend API
            # calls on full public-rule hydration. Missing metadata is UNKNOWN,
            # not an automatic contradiction. Explicit mismatches still reject.
            cert0 = candidate.get("equivalence_certificate") or {}
            ks0 = candidate.get("kalshi_signature") or {}
            ps0 = candidate.get("polymarket_signature") or {}
            needs_recovery = (
                str(cert0.get("resolution_rule_status") or "").upper() not in {"EXACT", "COMPATIBLE", "LOW_BASIS"}
                or any((ks0.get(k) and not ps0.get(k)) or (ps0.get(k) and not ks0.get(k))
                       for k in ("office_scope", "jurisdiction_region", "jurisdiction_district"))
            )
            if needs_recovery:
                if V29_SKIP_LATE_RULE_HYDRATION:
                    # V29 relaxed paper lane: trust the semantic matcher when
                    # evidence is merely missing. Explicit mismatches already
                    # present in the signatures are still rejected by the gate.
                    recovery_counts["late_rule_relaxed_without_hydration"] += 1
                    log.append("recovery", {"timestamp": time.time(), "ticker": ticker,
                        "subject": candidate.get("subject"), "state": "RELAXED",
                        "reason": "V29 semantic-trust lane: unresolved rule evidence is non-blocking"})
                else:
                    rstate, rreason = late_hydrate_candidate(candidate, pm, execution_recovery_cache, retries=cfg.api_retries)
                    recovery_counts[f"late_rule_{rstate.lower()}"] += 1
                    log.append("recovery", {"timestamp": time.time(), "ticker": ticker,
                        "subject": candidate.get("subject"), "state": rstate, "reason": rreason})
                    if rstate == "CONTRADICTION":
                        reason = f"execution audit: explicit contradiction after hydration: {rreason}"
                        rejection_counts[reason] += 1
                        log.append("signals", _signal_row_v8(candidate, signal, False, reason, settlement_days=rough_days, duration_bucket=bucket))
                        continue
                    if rstate == "UNRESOLVED" and not V29_RELAX_UNRESOLVED:
                        reason = f"execution audit: temporarily unresolved rule evidence: {rreason}"
                        rejection_counts[reason] += 1
                        log.append("signals", _signal_row_v8(candidate, signal, False, reason, settlement_days=rough_days, duration_bucket=bucket))
                        continue
        ...
        for p in proposals:
            ...
            if signal["net_per_contract"] < required_edge or signal["net_profit"] < cfg.min_trade_net_dollars:
                ...
                continue
            cert0 = candidate.get("equivalence_certificate") or {}
            ks0 = candidate.get("kalshi_signature") or {}; ps0 = candidate.get("polymarket_signature") or {}
            needs_recovery = (str(cert0.get("resolution_rule_status") or "").upper() not in {"EXACT", "COMPATIBLE", "LOW_BASIS"}
                or any((ks0.get(k) and not ps0.get(k)) or (ps0.get(k) and not ks0.get(k))
                       for k in ("office_scope", "jurisdiction_region", "jurisdiction_district")))
            if needs_recovery:
                if V29_SKIP_LATE_RULE_HYDRATION:
                    recovery_counts["preexecution_rule_relaxed_without_hydration"] += 1
                else:
                    rstate, rreason = late_hydrate_candidate(candidate, pm, execution_recovery_cache, retries=cfg.api_retries)
                    recovery_counts[f"late_rule_{rstate.lower()}"] += 1
                    if rstate == "CONTRADICTION":
                        rejection_counts["pre-execution recovery contradiction"] += 1
                        continue
                    if rstate == "UNRESOLVED" and not V29_RELAX_UNRESOLVED:
                        rejection_counts["pre-execution recovery unresolved"] += 1
                        continue
            gate_ok, gate_reason = _equivalence_edge_gate(candidate, signal, cfg)
            if not gate_ok:
                rejection_counts[gate_reason] += 1
        ...
        print(
            f"Cycle {cycle} | {elapsed:.1f}m | watch_checked={cycle_watch_checked}/{persistent_watch_size} | "
            f"proposals={len(proposals)} | open={len(portfolio.positions)} | "
...
def print_summary(portfolio: V8Portfolio, results, log: RunLoggerV8, rejection_counts, run_minutes, cfg: V8Config):
    ...
    print("\n" + "=" * 100)
    print("PAPER ENGINE V28.1 RESILIENT EXECUTION RECOVERY - CREDIBLE-PROFIT SUMMARY")
    print("=" * 100)
    ...
    print("\nV28 candidate recovery summary:")
    if recovery_counts:
        for reason, count in recovery_counts.most_common():
            print(f"  {count:6d}  {reason}")
    else:
        print("       0  no late recovery attempts")
    ...
    print(f"\nRun data: {log.root}")
    print("Paper only: V28.1 Resilient Execution Recovery contains no order-placement call.")


def main():
    # V15 restores V13's live Kalshi cache refresh. V14 could run against a
    # stale parquet even while event metadata was current, suppressing genuine
    # cross-platform pairs and making the portfolio appear artificially idle.
    ...
    ap.add_argument("--maker-max-active", type=int, default=20)
    ap.add_argument("--maker-total-cap", type=float, default=0.12)
    ...
    cfg = V8Config(
        ...
        max_watch_candidates=max(1, a.confirm_top), max_watch_universe=max(1, max(a.confirm_top, 220)),
