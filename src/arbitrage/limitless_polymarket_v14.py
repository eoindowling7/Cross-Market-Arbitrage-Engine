...
class LimitlessPolyConfig:
    enabled: bool = True
    max_settlement_days: float = 21.0
    max_pairs_per_scan: int = 200
    max_capital_fraction_per_trade: float = 0.08
    min_profit_dollars: float = 0.01
    min_return_on_capital: float = 0.0015
    safety_buffer_per_contract: float = 0.00075
    deadline_tolerance_seconds: float = 300.0
    max_qty: int = 200
    limitless_discovery_limit: int = 1200
    enrich_limitless_candidates: int = 300
...
def _threshold(text):
    ...
    # Accept both currency-prefixed thresholds and bare numeric crypto/equity
    # thresholds. Asset + direction still have to parse independently, so this
    # does not turn unrelated dates/IDs into exact-pair candidates.
    patterns = (
        r"\$\s*([0-9]+(?:\.[0-9]+)?)\s*([kKmM])?",
        r"(?:above|over|below|under|greater than|less than|higher than|lower than|exceed)\s+\$?\s*([0-9]+(?:\.[0-9]+)?)\s*([kKmM])?",
    )
    m = None
    for pat in patterns:
...
def _core_signature(m):
    """Asset/direction/threshold identity available even on sparse list rows."""
    ...
    if None in (asset,op,threshold):
        return None
    return asset,op,round(float(threshold),8)
...
def _enrich_limitless_rows(rows, poly_markets, cfg):
    """Enrich plausible sparse Limitless rows before requiring deadline/source.

    V13 required a complete pre-signature (including deadline) before detail
    fetching. On current Limitless list responses that can be circular because
    the deadline/oracle fields may only be present in the market-detail record.
    V14 first matches the safe asset/operator/threshold core, then fetches the
    detailed record and applies the original strict source + deadline checks.
    """
    now=time.time()
    poly_cores=set()
    for pm in poly_markets:
        core=_core_signature(pm)
        end=_poly_end(pm)
        if core and end and 0 < end-now <= cfg.max_settlement_days*86400:
            poly_cores.add(core)
    if not poly_cores:
        return rows,0,0
    ...
    for lm in rows:
        current=lm
        core=_core_signature(lm)
        sig=_signature(lm,"limitless")
        needs_detail = core in poly_cores and sig is None
        if needs_detail and attempted < cfg.enrich_limitless_candidates:
            slug=lm.get("slug")
            if slug:
                attempted += 1
                try:
...
def _pair_diagnostics(poly_markets, limitless_markets, cfg):
    now=time.time(); stats={
        ...
        "detail_fetch_attempts":0,"detail_fetch_successes":0,"lumy_markets":0,"near_misses":[],
    }
    poly=[]
    for pm in poly_markets:
...
def scan_limitless_polymarket(poly_markets, available_cash, bankroll, cfg: LimitlessPolyConfig, *, return_diagnostics=False):
    ...
    for a,b,sig in pairs:
        ...
        if near:
            ...
            if best_near is None or near["net_after_buffer"]>best_near["net_after_buffer"]: best_near=near
            push_top(stats["near_misses"], near, key="net_after_buffer", limit=10)
        out.extend(ops)
